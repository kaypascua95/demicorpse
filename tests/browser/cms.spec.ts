...
test('fragment media stays in drafts until published and opens from the visual feed', async ({page}, info) => {
  ...
  const video = await page.locator('#persistent-video').elementHandle();
  const fragmentHref = await page.locator('.fragment-marquee-card footer a').first().getAttribute('href');
  expect(fragmentHref).toBeTruthy();
  await page.goto(fragmentHref!);
  await expect(page).toHaveURL(new RegExp(`/fragments/${records[0].slug}import { test, expect, type Page } from '@playwright/test';
import { readFileSync } from 'node:fs';
